...
//	v.erase(p1, p2); // p1 ~ p2 의 구간에 있는 요소 제거
	...
	v.swap(v1); // v와 v1 의 버퍼 교환

	// 생각 보다 멤버 함수가 많지 않습니다!!
	// sort, find, reverse 하려면 어떻게 하나요 ???
	// => 알고리즘을 사용하면 되므로 멤버 함수가 많지 않습니다.

	// 멤버 함수는 대부분
	// => vector 자체의 메모리 관리와 연관된것이 많습니다.
	// => 알고리즘은 컨테이너를 모르기 때문에 메모리 관련 작업 못함.!
}